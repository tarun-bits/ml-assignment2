...
class DecisionTreeClassifierPipeline():
    def __init__(self, preprocessor: ColumnTransformer, train_test_split: TrainTestSplit):
        self.model_lib = DecisionTreeClassifier(random_state=42, max_depth=10,criterion='entropy' )
    ...
    def model_metrics(self, model):
        ...
        # DecisionTreeClassifier supports predict_proba; handle gracefully if not available
        try:
    ...
    def save_pkl_model(self, model):
        file_path = Path(__file__).with_name('decision_tree_model.pkl')
